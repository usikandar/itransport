...
var seatBoxHandler = function (el) {
	...
	//check valid key
	if(seatNum =="@TV" || seatNum=="@WR" || seatNum=="@PT" || parseInt(seatNum)>0 || seatNum.includes("@SL"))
		isValid =true;
	else
		isValid =false;
...
//console.log(lowerSeats.length,'lowerSeatsLength');
	// sleeper seat
	i=0;
	var lowerSleepSeats = $('#seat-design-lower')
                    .find('.vehicle-sleep.selected')
                    .map(function() {
						i++;
						if(i==1)
 ...
 //                    .get();
  ...
  i=0;
	var upperSleepSeats = $('#seat-design-upper')
                    .find('.vehicle-sleep.selected')
                    .map(function() {
						i++;
						if(i==1)
...
lowerSeats = $('#seat-design-lower')
			...
			.get();
lowerSleepSeats = $('#seat-design-lower')
			.find('.vehicle-sleep.selected')
			.map(function() {
				$(this).toggleClass('selected');
			})
...
upperSeats = $('#seat-design-upper')
			...
			.get();		
upperSleepSeats = $('#seat-design-upper')
			.find('.vehicle-sleep.selected')
			.map(function() {
				$(this).toggleClass('selected');
			})
