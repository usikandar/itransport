    ...
    for (var rnum = rowStart; rnum <= rowEnd; rnum++) {
      ...
      for (var cnum = colStart; cnum <= colEnd; cnum++) {
        ...
        if(rowCount == 13 || rowCount == 14){
          ...
          seatDesignLowerHTML = seatDesignLowerHTML + "<td style='"+style+"' rel='" + rnum + cnum + "' id='designLower" + rnum + cnum + "' class='vehicle-seat "+classToappend+" "+selected+"' onclick='"+onclickFunction+"'>"+seatID+"</td>";//seatDesignHandler(this)
...
var generatXml  = function (el,dimensionType) {
  ...
  $.ajax({
    type: "POST",
    url: "Ajax.cfm",
    data: {ajaxAction: 'saveBusStructure', dimensiontype: dimensionType, xmlData: xmlStringFinal, VechicleTypeID: vehicleTypeId, VersionID: versionId},
    returnformat:'json',
...
var generatUpdateXml  = function (el,dimensionType) {
  ...
  $.ajax({
    type: "POST",
    url: "Ajax.cfm",
    data: {ajaxAction: 'updateBusStructure', dimensiontype: dimensionType, xmlData: xmlStringFinal, VechicleTypeID: vehicleTypeId, VersionID: versionId},
    returnformat:'json',
      success: function (data) {
        if(data!==''){
          alert(data);
